...
目前 code structure、資料流與已知耦合點分析請先參考 [`FERMI/docs/CODE_STRUCTURE_ANALYSIS.md`](FERMI/docs/CODE_STRUCTURE_ANALYSIS.md)。
...
## Paper-style reconstruction mode（最小可執行近似）

本模式對齊 [`references/FERMI.pdf`](references/FERMI.pdf) 的 LaMP setup 與 Appendix 模板重點，
但不在本子任務執行全矩陣實驗。

目前倉內資料以 validation snapshot 與 paper-style reconstruction 為主，常見根目錄包含 [`FERMI/LaMP_validation_only_seed42/`](FERMI/LaMP_validation_only_seed42)、[`FERMI/LaMP_planA_seed42/`](FERMI/LaMP_planA_seed42)、[`FERMI/LaMP3_rate_time_paper_style_seed42/`](FERMI/LaMP3_rate_time_paper_style_seed42)；若執行 CLI，通常需要依實際資料位置指定 `--data_root`。

### 主要對齊內容

- `K=4`, `L=5`, `T=10`
- `rop_n_tilde=3`
- evaluator M = `gpt-3.5-turbo`，optimizer Mopt = `gpt-4`
...
- `model_M_name`：Evaluator 模型名稱（目前預設 `gpt-3.5-turbo`）
- `model_Mopt_name`：Optimizer 模型名稱（目前預設 `gpt-4`）
...
- 結構分析與設定覆蓋：[`FERMI/docs/CODE_STRUCTURE_ANALYSIS.md`](FERMI/docs/CODE_STRUCTURE_ANALYSIS.md)
- 架構總覽：[`FERMI/docs/ARCHITECTURE.md`](FERMI/docs/ARCHITECTURE.md)
- 輸出格式規範：[`FERMI/docs/OUTPUT_SPEC.md`](FERMI/docs/OUTPUT_SPEC.md)
